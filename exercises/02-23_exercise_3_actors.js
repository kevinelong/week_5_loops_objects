...
function startsWith(partialName){
    for(let i=0; i < academyMembers.length; i++){
        let member = academyMembers[i];
        if(member.name.indexOf(partialName) == 0 ){ //startsWith
            console.log(member.name);
        }
    }
}
function filmStartsWith(partialName){
    for(let i=0; i < academyMembers.length; i++){
        ...
        for(let f=0; f < member.films.length; f++){
            ...
            if(film.indexOf(partialName) == 0 ){ //startsWith
                count++;
            }
        }
        if (count > 0){
...
console.log(`HARDER: Which Academy Members have been in a film that starts with "A"`);
filmStartsWith("A")
